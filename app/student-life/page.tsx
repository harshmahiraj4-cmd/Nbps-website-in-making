    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="pt-[164px] flex-1">
          ...
          <div className="relative z-10 max-w-container mx-auto px-8 w-full">
            <div className="max-w-2xl">
              ...
              <div className="flex flex-wrap gap-4">
                <a href="#sports" className="bg-white text-primary font-semibold px-8 py-4 rounded-lg hover:bg-slate-100 transition-colors shadow-lg" style={serifStyle}>Sports & Games</a>
        ...
        <section className="max-w-container mx-auto px-8 py-14 -mt-12 relative z-20">
          <div className="grid grid-cols-2 md:grid-cols-4 bg-white shadow-xl rounded-xl border border-gray-100 dark:border-gray-700 divide-x divide-y md:divide-y-0 divide-gray-100 overflow-hidden">
            {[
              ...
              <div key={stat.label} className="p-8 text-center">
                <div className="text-4xl font-bold text-primary mb-1" style={serifStyle}>{stat.value}</div>
                <div className="text-secondary font-semibold text-sm uppercase tracking-widest" style={serifStyle}>{stat.label}</div>
              </div>
            ))}
          </div>
        </section>
        ...
        <section id="sports" className="max-w-container mx-auto px-8 py-stack-lg">
          <div className="mb-10">
            <span className="text-secondary font-semibold text-sm mb-2 block uppercase tracking-widest" style={serifStyle}>Physical Excellence</span>
            <h2 className="text-4xl font-bold text-primary mb-4" style={serifStyle}>Sports & Physical Education</h2>
            <p className="text-lg text-on-surface-variant max-w-2xl" style={serifStyle}>We believe a healthy body builds a healthy mind. NBPS offers a wide range of sports activities to keep every student active, disciplined, and confident.</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {activities.map((act) => (
              <div key={act.name} className="bg-white p-6 rounded-xl border border-gray-100 dark:border-gray-700 hover:border-amber-400 hover:shadow-md transition-all group">
                <div className="w-12 h-12 bg-blue-950 rounded-lg flex items-center justify-center mb-4 group-hover:bg-secondary-container transition-colors">
                  <span className="material-symbols-outlined text-amber-400 group-hover:text-blue-950">{act.icon}</span>
                </div>
                <h3 className="text-xl font-bold text-blue-950 mb-2" style={serifStyle}>{act.name}</h3>
                <p className="text-sm text-on-surface-variant" style={serifStyle}>{act.desc}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        {/* Clubs */}
        <section id="clubs" className="bg-surface-container-low dark:bg-gray-900 py-stack-lg">
          <div className="max-w-container mx-auto px-8">
            <div className="text-center mb-12">
              <span className="text-secondary font-semibold text-sm block mb-3 uppercase tracking-widest" style={serifStyle}>Find Your Passion</span>
              <h2 className="text-4xl font-bold text-primary mb-4" style={serifStyle}>Clubs & Co-curricular Activities</h2>
              <p className="text-lg text-on-surface-variant max-w-2xl mx-auto" style={serifStyle}>Our co-curricular clubs nurture talent, creativity, and social responsibility beyond the classroom.</p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {clubs.map((club) => (
                <div key={club.name} className="bg-white p-6 rounded-xl border border-gray-100 dark:border-gray-700 hover:border-amber-400 hover:shadow-md transition-all group">
                  <div className="w-12 h-12 bg-surface-container-low rounded-lg flex items-center justify-center mb-4 group-hover:bg-secondary-container transition-colors">
                    <span className="material-symbols-outlined text-blue-950">{club.icon}</span>
                  </div>
                  <div className="text-xs text-secondary font-semibold mb-2 uppercase tracking-wide" style={serifStyle}>{club.members}</div>
                  <h4 className="font-bold text-blue-950 mb-2 text-lg" style={serifStyle}>{club.name}</h4>
                  <p className="text-sm text-on-surface-variant" style={serifStyle}>{club.desc}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <section className="max-w-container mx-auto px-8 py-stack-lg">
          <div className="flex flex-col lg:flex-row gap-gutter items-center">
            ...
            <div className="lg:w-1/2">
              <span className="text-secondary font-semibold text-sm block mb-3 uppercase tracking-widest" style={serifStyle}>Annual Highlights</span>
              <h2 className="text-4xl font-bold text-blue-950 mb-6" style={serifStyle}>School Celebrations & Events</h2>
              <p className="text-lg text-on-surface-variant mb-6" style={serifStyle}>Throughout the year, NBPS comes alive with national celebrations, cultural events, and academic programmes that build pride, patriotism, and community spirit.</p>
              <ul className="space-y-4 mb-8">
                {[
                  ...
                  <li key={item.text} className="flex items-start gap-3">
                    <div className="w-8 h-8 bg-secondary-container rounded-lg flex items-center justify-center flex-shrink-0 mt-0.5">
                      <span className="material-symbols-outlined text-sm text-blue-950">{item.icon}</span>
                    </div>
                    <span className="text-sm text-on-surface-variant leading-relaxed" style={serifStyle}>{item.text}</span>
                  </li>
                ))}
              </ul>
              <Link href="/contact" className="inline-flex items-center gap-2 px-6 py-3 bg-blue-950 text-white rounded-lg font-semibold hover:opacity-90 transition-all" style={serifStyle}>
        ...
        {/* Values Block */}
        <section className="bg-blue-950 py-stack-lg px-8">
          <div className="max-w-container mx-auto">
            <div className="text-center mb-12">
              <h2 className="text-4xl font-bold text-white mb-3" style={serifStyle}>Our Core Values</h2>
              <p className="text-blue-200 text-lg max-w-xl mx-auto" style={serifStyle}>We shape not just students, but responsible citizens grounded in these principles.</p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 text-center">
              {[
                ...
                <div key={item.title} className="p-8">
                  ...
                  <p className="text-blue-200 text-sm" style={serifStyle}>{item.desc}</p>
                </div>
              ))}
            </div>
          </div>
